...
class TagSelect extends Component {
  ...
  handleTagChange = (value, checked) => {
    const { value: StateValue, single } = this.state;
  ...
  render() {
    ...
    const { title, children, hideCheckAll, className, style, expandable } = this.props;
    ...
    return (
      <div className={cls} style={style}>
        {title && 
          <span style={{marginRight: 20, fontWeight: "bold"}}>{title}:</span>
        }
        {hideCheckAll ? null : (
